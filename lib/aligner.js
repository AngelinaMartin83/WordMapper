...
export class ForcedAligner {
  ...
  getPhonemes(word) {
    const raw = this._resolveDictionaryIpa(word);
  ...
  alignWord(word, overrideIpaStr = null) {
    ...
    const dictionaryIpa = this._resolveDictionaryIpa(exactKey);
    ...
    const result = this.align(this._normalizeWordSeparators(key), ipaText);
    return { ...result, ipaAligned: phonemes, ipaDisplay };
  }

  _lookupDictionaryIpa(word) {
    const exactKey = String(word || '').trim();
    return this.ipaDictRaw[exactKey] || this.ipaDictRawLower[exactKey.toLowerCase()] || '';
  }

  _resolveDictionaryIpa(word) {
    const exactKey = String(word || '').trim();
    const exactIpa = this._lookupDictionaryIpa(exactKey);
    if (exactIpa) return exactIpa;

    const parts = exactKey
      .split(/[\s\p{Dash_Punctuation}]+/u)
      .map(part => part.trim())
      .filter(Boolean);
    if (parts.length < 2) return '';

    const ipaParts = parts.map(part => this._lookupDictionaryIpa(part));
    return ipaParts.every(Boolean) ? ipaParts.join(' ') : '';
  }

  _normalizeWordSeparators(word) {
    return String(word || '').replace(/[\s\p{Dash_Punctuation}]+/gu, separators => {
      const dash = [...separators].find(char => /\p{Dash_Punctuation}/u.test(char));
      return dash || ' ';
    });
  }
  ...
  _isWordSeparator(char) {
    return /[\s\p{Dash_Punctuation}]/u.test(char || '');
  }

  _hasWordSeparator(text) {
    return /[\s\p{Dash_Punctuation}]/u.test(String(text || ''));
  }
