...
export class ForcedAligner {
  constructor() {
    ...
    this.cost_force = 10;
    this.ipaDict = {};
    this.ipaDictRaw = {}; // 保存原始IPA字符串用于展示
    this._initG2P();
  }
  ...
  loadIpaDictionary(obj){
    ...
    for (const k of Object.keys(obj || {})) {
      ...
      let sOrig = String(val).trim();
      sOrig = sOrig.replace(/^\s*\/|\/\s*$/g, ''); // 去掉外层斜杠 /.../
      raw[key]  = sOrig;            // 展示用：保留原始
      norm[key] = [sOrig];          // 存一份，兼容旧逻辑
  ...
  sanitizeIPAForAlignment(ipaStr) {
    ...
    // ✅ 若末尾含有 (r)，先去掉，仅用于映射计算
  s = s.replace(/\(r\)$/i, '');
     // 1) 去重音、音节点、连结符、斜杠、空白括号等
    s = s.replace(/[ˈˌ\.͡\/\[\]\(\)\{\}]/g, '');
    // 2) 去常见附加记号（送气 ʰ、卷舌 ˞、软腭化 ʷ、腭化 ʲ 等）
    s = s.replace(/[ʰʷʲ˞ˀˁ̚˕˖˔˩˥˦˧˨]/g, '');
    // 3) 去所有 Unicode 组合附加符（Mn 类别）
    s = s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').normalize('NFC');
    // 注意：保留长音符号“ː”，若你要去掉可追加： s = s.replace(/ː/g, '');
    return s.trim();
    }
  splitIPA(ipa){
    ...
    const phonemeList = [
      ...
      'p','b','t','d','k','ɡ','m','n','ŋ','f','v','θ','ð','s','z','ʃ','ʒ','h','l','r','j','w','tr','ts','dz','dr','i','u'
    ].sort((a,b)=>b.length-a.length);
    let clean=(ipa||'');
        // 把多元音连串 ɪəʊ 改成 ɪ·əʊ
  clean = clean.replace(/ɪəʊ/g, 'ɪ·əʊ');
    const out=[]; let i=0;
    while(i<clean.length){
      if (clean[i] === '·') { i++; continue; } // 跳过人为加的分隔符
      let ok=false;
  ...
  getPhonemes(word){
    ...
    const raw=this.ipaDictRaw[w];
    if(!raw) return null;
    const sanitized = this.sanitizeIPAForAlignment(raw);
    return this.splitIPA(sanitized);
  }

  isValid(g,p){ return this.g2p[g] && this.g2p[g].has(p); }

  alignWord(word, overrideIpaStr=null){
    // 展示用原始 IPA（优先显示用户覆盖的原始串；否则显示词库原始）
    const ipaDisplay = (overrideIpaStr != null && overrideIpaStr !== '')
      ? String(overrideIpaStr)
      : (this.ipaDictRaw[(word||'').toLowerCase()] || '');
    // 对齐用净化后的 IPA
    const ipaForAlign = (overrideIpaStr != null && overrideIpaStr !== '')
      ? this.sanitizeIPAForAlignment(overrideIpaStr)
      : this.sanitizeIPAForAlignment(this.ipaDictRaw[(word||'').toLowerCase()] || '');
    const phonemes = this.splitIPA(ipaForAlign);
    // ✅ 如果词典里没有该单词，直接报错，不再尝试对齐
    const hasInDict = (word || '').toLowerCase() in this.ipaDictRaw;
    if (!hasInDict && (!overrideIpaStr || overrideIpaStr.trim() === '')) {
      throw new Error(`未在词典中找到单词 “${word}”`);
    }
  ...
  _initG2P(){
    ...
    add('s', ['s','z','ʃ','ʒ']);
    add('t', ['t']);
    add('u', ['uː','u','ɪ','ʊ','ə','ʌ','juː','ju','jʊ','jʊə','ʒʊə']);
    add('v', ['v']);
    add('w', ['w','（沉默）']);
